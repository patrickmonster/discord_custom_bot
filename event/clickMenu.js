...
module.exports = async function(interaction) {
	...
	const command = client.system_mu.get(commend);
	if (command) {// 이벤트 여부 확인
		console.log(`${guild ? guild.id : '아이디가 없음'}/${channelId}`, commend, args, '매뉴 이벤트 처리', interaction.values, user.tag, twitch_auth_list.includes(command));
	}
	else {
